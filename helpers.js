...
class Helpers {
    ...
    static createElement(elementName, attributes, createClass = true) {
        let el = document.createElementNS(Constants.SVG_NS, elementName);

        // Create a unique ID for the element so we can acquire the correct shape controller when the user drags the shape.
        el.setAttributeNS(null, "id", Helpers.uuidv4());

        if (createClass) {
            // Create a class common to all shapes so that, on file load, we can get them all and re-attach them to the mouse controller.
